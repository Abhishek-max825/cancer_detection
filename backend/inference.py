...
from grad_cam import visualize_gradcam, get_gradcam_layer, GradCAM
...
class EnsembleCancerPredictor:
    ...
    def generate_gradcam(self, image: Image.Image) -> str:
        """
        Generate Grad-CAM using the strongest model (DenseNet) or fallback
        """
        ...
        target_layer = get_gradcam_layer(target_model)
        gradcam = GradCAM(target_model, target_layer)
        
        target_model.eval()
        heatmap = gradcam.generate_cam(tensor)
        overlaid = gradcam.overlay_heatmap(original_array, heatmap, alpha=0.4)
